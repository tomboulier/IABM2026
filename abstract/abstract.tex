...
\IABMauthors{T. Boulier\textsuperscript{1,2}, A. Moreau-Gaudry\textsuperscript{2}}
\IABMaffiliations{%
...
claire entre variabilité et qualité de génération. L'analyse visuelle (figure 1)
confirme les scores FID élevés pour RetinaMNIST, mais révèle des incohérences
entre FID et qualité perçue pour d'autres datasets (BloodMNIST vs OrganCMNIST).
Ces résultats suggèrent que la relation entre diversité des données et facilité
d'apprentissage génératif est plus complexe qu'anticipée. Plusieurs limites doivent
être considérées : entraînement limité à une seule itération, faible résolution
(28$\times$28), et choix de la métrique de variabilité. Des tests avec 10 itérations
sur RetinaMNIST montrent une amélioration notable comparée au résultat actuel qui
n'est que de la génération de bruit. Les travaux en cours visent des
campagnes de calcul sur une plateforme de calcul haute performance. Ils
comprendront des entraînements plus longs, des résolutions supérieures, des
métriques alternatives, d'autres jeux de données (dont les versions 3D de MedMNIST),
et d'autres architectures génératives , comme les GAN \cite{gan} ou les  VAE \cite{vae}).
...
\begin{figure}[h!]
    ...
    itération (droite) pour trois datasets : OrganCMNIST, BloodMNIST et
    RetinaMNIST.}
\end{figure}
